...
description: >
  ...
  or any multi-section content this session, ingest it into playbooks before the session ends and
  update the associated note. Also invoke when retrieving an existing strategy or procedure.
  The AI Playbooks Server (http://localhost:8001) is the living source of truth for structured knowledge.
applyTo: "**"
---
...
playbooks store multi-section documents that agents consult selectively.

> If `playbooks` is not installed, follow [INSTALL.md](./INSTALL.md). If the CLI is unusable, see [FALLBACK.md](./FALLBACK.md) for curl commands.
...
**Rule of thumb:** if it needs headers, it's a playbook.
...
> **Playbooks are an extension of agent memory.** If you don't ingest it, it's lost. Don't wait for the user to ask.

### Always Update (when content changes)

- You revised a plan or strategy already stored as a playbook → run `playbooks update <slug> --file <updated-file>`
- You completed a phase described in a playbook → update the relevant section so future agents see current state
- A previously documented approach was found to be wrong or outdated → update or mark stale

### Retrieve a playbook when:
- A note contains a slug pointer to a playbook
- You need a full procedure, not just a one-liner fact
- Semantic search returns a section result pointing to a playbook
...
# Ingest a new playbook from a file
playbooks ingest auth.md --slug "junkyard-scraper/auth" --description "OAuth and session handling" --tags "scraper,auth"
...
playbooks ls --status stale

# Browse structure — cheap, do before get
playbooks toc junkyard-scraper/auth
playbooks children junkyard-scraper

# Retrieve content
playbooks get junkyard-scraper/auth              # full document
playbooks get junkyard-scraper/auth#oauth-flow   # single section only

# Semantic search (searches section embeddings)
playbooks search "token refresh handling" --scope junkyard-scraper
playbooks search "cloudflare bypass" --limit 10

# Deprecate
playbooks stale junkyard-scraper/auth
playbooks activate junkyard-scraper/auth
...
1. Run `playbooks slugs --scope <relevant-scope>` to check what already exists
2. If a related document exists, decide:
   ...
   - **Update** — use `playbooks update` instead of ingest
...
4. After ingesting, check for an existing note before creating one:
   ...
   - If a relevant note exists: `notes update <id> --content "<existing content>. See playbooks: <slug>"` (append the pointer, preserve existing content)
   - If no note exists: `notes add "[topic]" "Detailed procedure documented. See playbooks: <slug>" --tags <tags>`
...
- Ingest scratchpad content, session logs, or in-progress notes — playbooks are polished reference documents
- Use playbooks as a substitute for notes — keep learnings in notes, larger documents in playbooks
...
Session produces a multi-step strategy
  → Agent ingests to playbooks: playbooks ingest strategy.md --slug "topic/strategy"
  → Agent searches for existing note: notes search "topic strategy"
  → If found: notes update <id> --content "<existing content>. See playbooks: topic/strategy"
  → If not found: notes add "topic strategy" "Full procedure documented. See playbooks: topic/strategy" --tags topic

Future session needs the strategy
  → notes search "topic strategy"  → finds pointer note with slug
  → playbooks toc topic/strategy   → reviews structure
  → playbooks get topic/strategy#relevant-section  → pulls only what's needed
```
